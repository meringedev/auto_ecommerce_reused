(() => {
  ...
  var require_jquery = __commonJS({
    "node_modules/jquery/dist/jquery.js"(exports, module) {
      (function(global7, factory) {
        "use strict";
        if (typeof module === "object" && typeof module.exports === "object") {
          module.exports = global7.document ? factory(global7, true) : function(w) {
            if (!w.document) {
          ...
          Expr = jQuery.expr = {
            ...
            filter: {
              ...
              PSEUDO: function(pseudo, argument) {
                var args2, fn = Expr.pseudos[pseudo] || Expr.setFilters[pseudo.toLowerCase()] || find.error("unsupported pseudo: " + pseudo);
                if (fn[expando]) {
                  return fn(argument);
                }
                if (fn.length > 1) {
          ...
          var firing, memory, fired, locked, list = [], queue = [], firingIndex = -1, fire = function() {
            ...
            add: function() {
              if (list) {
                ...
                (function add(args2) {
                  jQuery.each(args2, function(_, arg) {
                    if (isFunction2(arg)) {
        ...
        jQuery.event = {
          ...
          dispatch: function(nativeEvent) {
            var i2, j, ret, matched, handleObj, handlerQueue, args2 = new Array(arguments.length), event = jQuery.event.fix(nativeEvent), handlers = (dataPriv.get(this, "events") || /* @__PURE__ */ Object.create(null))[event.type] || [], special = jQuery.event.special[event.type] || {};
            args2[0] = event;
        ...
        function domManip(collection, args2, callback, ignored) {
          args2 = flat(args2);
          var fragment, first, scripts, hasScripts, node, doc, i2 = 0, l = collection.length, iNoClone = l - 1, value2 = args2[0], valueIsFunction = isFunction2(value2);
          if (valueIsFunction || l > 1 && typeof value2 === "string" && !support.checkClone && rchecked.test(value2)) {
            return collection.each(function(index) {
              var self2 = collection.eq(index);
              if (valueIsFunction) {
                args2[0] = value2.call(this, index, self2.html());
              }
        ...
        jQuery.fn.extend({
          val: function(value2) {
            ...
            return this.each(function(i2) {
              ...
              if (valueIsFunction) {
                val = value2.call(this, i2, jQuery(this).val());
              } else {
                val = value2;
              }
              if (val == null) {
        ...
        jQuery.extend({
          valHooks: {
            ...
            select: {
              ...
              set: function(elem, value2) {
                var optionSet, option, options = elem.options, values = jQuery.makeArray(value2), i2 = options.length;
                while (i2--) {
                  option = options[i2];
  ...
  var require_config = __commonJS({
    "config.js"(exports, module) {
      var global7 = {
        ngrok_api_url: "https://7fa3-2a00-23c5-eea0-7601-141d-75a9-c7ce-9ec8.ngrok-free.app",
        ngrok_frontend_url: "https://ca0f-2a00-23c5-eea0-7601-141d-75a9-c7ce-9ec8.ngrok-free.app",
        options: {
      ...
      module.exports = { global: global7 };
    }
  });

  // js/shared/ES/shared_gen_func.js
  var $, axios2, global2, gen_func;
  var init_shared_gen_func = __esm({
    "js/shared/ES/shared_gen_func.js"() {
      $ = Promise.resolve().then(() => __toESM(require_jquery(), 1));
      axios2 = Promise.resolve().then(() => (init_axios2(), axios_exports));
      ({ global: global2 } = Promise.resolve().then(() => __toESM(require_config(), 1)));
      gen_func = {
        ...
        render_user_state: function(state, functs) {
          ...
          args = this.get_prop(functs, `args_${i}`);
          if (funct !== null) {
        ...
        render_auth_page: function(state, functs) {
          inner_functs = {
            ...
            args_1: [functs.render_1, functs.render_2],
            funct_2: this.redirect,
            args_2: [`${global2.ngrok_api_url}/verify/`],
            funct_3: this.redirect,
            args_3: [`${global2.ngrok_api_url}/login/`]
          };
  ...
  var require_axios = __commonJS({
    "node_modules/axios/dist/browser/axios.cjs"(exports, module) {
        ...
        function defaultVisitor(value2, key, path) {
          ...
          if (value2 && !path && typeof value2 === "object") {
            if (utils$1.endsWith(key, "{}")) {
              key = metaTokens ? key : key.slice(0, -2);
              value2 = JSON.stringify(value2);
            } else if (utils$1.isArray(value2) && isFlatArray2(value2) || (utils$1.isFileList(value2) || utils$1.endsWith(key, "[]")) && (arr = utils$1.toArray(value2))) {
          ...
          formData.append(renderKey2(path, key, dots), convertValue(value2));
          return false;
        }
        const stack = [];
      ...
      prototype3.append = function append2(name, value2) {
        this._pairs.push([name, value2]);
      };
      ...
      function toURLEncodedForm2(data2, options) {
        return toFormData3(data2, new platform.classes.URLSearchParams(), Object.assign({
          visitor: function(value2, key, path, helpers) {
      ...
      var defaults2 = {
        ...
        transformRequest: [function transformRequest2(data2, headers) {
          ...
          const isFormData3 = utils$1.isFormData(data2);
          if (isFormData3) {
            return hasJSONContentType ? JSON.stringify(formDataToJSON2(data2)) : data2;
          }
          ...
          if (isObjectPayload) {
            ...
            if ((isFileList3 = utils$1.isFileList(data2)) || contentType.indexOf("multipart/form-data") > -1) {
              const _FormData = this.env && this.env.FormData;
              return toFormData3(
      ...
      function normalizeValue2(value2) {
        ...
        return utils$1.isArray(value2) ? value2.map(normalizeValue2) : String(value2);
      }
      ...
      function matchHeaderValue2(context, value2, header, filter2, isHeaderNameFilter) {
      ...
      utils$1.reduceDescriptors(AxiosHeaders3.prototype, ({ value: value2 }, key) => {
        let mapped = key[0].toUpperCase() + key.slice(1);
        return {
          get: () => value2,
          set(headerValue) {
      ...
      utils$1.forEach(knownAdapters2, (fn, value2) => {
        if (fn) {
          try {
            Object.defineProperty(fn, "name", { value: value2 });
          } catch (e) {
      ...
      var axios7 = createInstance2(defaults$1);
      axios7.Axios = Axios$1;
      axios7.CanceledError = CanceledError3;
      axios7.CancelToken = CancelToken$1;
      axios7.isCancel = isCancel3;
      axios7.VERSION = VERSION3;
      axios7.toFormData = toFormData3;
      axios7.AxiosError = AxiosError3;
      axios7.Cancel = axios7.CanceledError;
      axios7.all = function all3(promises) {
        return Promise.all(promises);
      };
      axios7.spread = spread3;
      axios7.isAxiosError = isAxiosError3;
      axios7.mergeConfig = mergeConfig3;
      axios7.AxiosHeaders = AxiosHeaders$1;
      axios7.formToJSON = (thing) => formDataToJSON2(utils$1.isHTMLForm(thing) ? new FormData(thing) : thing);
      axios7.getAdapter = adapters.getAdapter;
      axios7.HttpStatusCode = HttpStatusCode$1;
      axios7.default = axios7;
  ...
  function main_load() {
    axios3.get(`${global3.ngrok_api_url}/cart/`, global3.options).then((res) => {
      const shopping_cart = res.shopping_cart;
      const shopping_cart_items = res.shopping_cart_items;
      is_full_html_load(shopping_cart, shopping_cart_items, { empty: true });
    }).catch((error) => {
      console.log(error);
    });
  }
  ...
  function get_conts() {
    if (typeof is_full === "undefined") {
      const is_full2 = gen_func.url_check("shopping-cart");
    }
    if (is_full) {
    ...
    return { cont, cont_prefix };
  }
  function html_items_global(data2, cont_prefix2, $object2) {
    $object2.find(`${cont_prefix2}_product_global`).each(() => {
      let field_value = $2(this).data("insert-values");
      if (field_value === "quantity" && $2(this).data("is-selection")) {
        $2(this).find("option:selected").prop("selected", false);
        $2(this).find(`option[value='${data2.quantity}']`).prop("selected", true);
      } else {
        let value2 = gen_func.get_prop(data2, field_value);
        $2(this).text(value2);
      }
    });
  }
  function html_items(data2, empty = false) {
    const conts = get_conts();
    const cont2 = conts.cont;
    const cont_prefix2 = conts.cont_prefix;
    const html = $2(cont2).first().clone();
    if (empty === true) {
      $2(cont2).empty();
    }
    for (let item of data2) {
      ...
      html.find(`${cont_prefix2}_product_img`).text(`<img src="${img_location}">`);
      html_items_global(item, cont_prefix2, html);
      html.show();
      let output = `${html}
<hr />`;
      $2(output).appendTo(cont2);
    }
    $2(cont2).find("hr").last().remove();
  }
  function mini_html(data2) {
    $2(".shop_options_cart_total").text(`TOTAL: ${data2.total}`);
  }
  function main_html(data2) {
    const total_quantity = data2.total_quantity;
    const total_quantity_text = quantity_text(total_quantity);
    $2("#shopping_cart_summary_qty").text(total_quantity_text);
    $2("#shopping_cart_summary_subtotal").text(`R${data2.subtotal}`);
    $2("#shopping_cart_summary_vat").text(`R${data2.vat}`);
    $2("#shopping_cart_summary_total").text(`R${data2.total}`);
  }
  function html_loader(main_funct, data2, opts = null, data_items = null) {
    let empty = false;
  ...
  function is_full_html_load(data2, data_items = null, opts = null) {
    let main_funct;
    if (typeof is_full === "undefined") {
      const is_full2 = gen_func.url_check("shopping-cart");
    }
    if (is_full) {
      main_funct = main_html;
    } else {
      main_funct = mini_html;
    }
    html_loader(main_funct, opts, data2, data_items);
  }
  function add_cart_item(data2) {
    axios3.post(`${global3.ngrok_api_url}/cart/`, data2, global3.options).then((res) => {
      const data3 = res.data;
      const flag = data3.flag;
      if (typeof flag !== "undefined" && flag === "updated") {
        inner_update_item(data3);
      } else {
        is_full_html_load(data3.shopping_cart, data3.shopping_cart_items);
      }
    }).catch((err) => {
      console.log(err);
    });
  ...
  function remove_cart_item(e) {
    const shopping_cart_item_id2 = $2(this).parents().eq(5).data("shopping-cart-item-id");
    axios3.delete(`${global3.ngrok_api_url}/cart/${shopping_cart_item_id2}/`, global3.options).then((res) => {
      const data2 = res.data;
      is_full_html_load(data2.shopping_cart);
      $2(this).remove();
    }).catch((err) => {
      console.log(err);
    });
  }
  function inner_update_item(data2) {
    is_full_html_load(data2.shopping_cart);
    const conts = get_conts();
    const cont_prefix2 = conts.cont_prefix;
    $object = $2(conts.cont).find(`[data-shopping-cart-item-id='${shopping_cart_item_id}']`);
    html_items_global(data2.shopping_cart_items, cont_prefix2, $object);
  }
  function update_item(data2) {
    shopping_cart_item_id = data2.shopping_cart_item_id;
    delete data2.shopping_cart_item_id;
    axios3.patch(`${global3.ngrok_api_url}/cart/${shopping_cart_item_id}/`, global3.options).then((res) => {
  ...
  function change_on_quantity(e) {
    const quantity = $2(this).find("option:selected").val();
    const shopping_cart_item_id2 = $2(this).parents().eq(5).data("shopping-cart-item-id");
    const data2 = {
      quantity,
      shopping_cart_item_id: shopping_cart_item_id2
    };
    update_item(data2);
  }
  var $2, axios3, check_user2, global3, is_full, user_state;
  var init_shopping_cart = __esm({
    "js/standard/shopping_cart.js"() {
      init_shared_gen_func();
      $2 = require_jquery();
      axios3 = require_axios();
      ({ check_user: check_user2 } = (init_sens(), __toCommonJS(sens_exports)));
      ({ global: global3 } = require_config());
      is_full = gen_func.url_check("shopping-cart");
      user_state = [false, false];
      $2(() => {
        user_state = check_user2();
        gen_func.return_auth_page(user_state, { render_1: main_load });
      });
      $2(document).on("change", ".shopping_cart_product_qty_select", change_on_quantity);
      $2(document).on("change", ".shopping_cart_product_remove_btn, shop_options_cart_product_remove_btn", remove_cart_item);
    }
  });
  ...
  function detect_amount_for_config_price(product_price) {
    let product_amount = $3("#quantity_select option:selected").val();
    console.log(product_amount);
    console.log(Number(product_amount));
    let amount = product_price * product_amount;
    $3(".product_price_2").text(`R${amount}`).data("cal-price", amount);
  }
  function product_select_btn_type(e) {
    console.log("clicked!!");
    $3(".product_select_type").removeClass("active");
    $3(this).addClass("active");
    let product_config_price = $3(this).data("product-config-price");
  ...
  function get_products_page(product_id) {
    const product_url = `${global4.ngrok_api_url}/products/${product_id}/`;
    axios4.get(product_url, global4.options).then((res) => {
      ...
      const product_id2 = product_details.product_id;
      const img = product_details.product_img;
      const img_url = `/public/${img}`;
      const img_html = `<img src="${img_url}" style="width: 100%;">`;
      $3(".product_img_cont").empty().append(img_html);
      $3(".product_name").text(product_details.name);
      $3("#product_info_table_data_dimensions").text(`${product_details.dimension_w}cm(w) x ${product_details.dimension_l}cm(l) x ${product_details.dimension_h}cm(h)`);
      let weight_type = product_details.weight_type;
      if (weight_type === null) {
        weight_type = "kg";
      }
      $3("#product_info_table_data_weight").text(`${product_details.weight}${product_details.weight_type}`);
      $3("#product_info_table_data_warranty").text(product_details.warranty);
      const is_repairable = product_details.is_repairable;
      let is_rep_html;
      if (is_repairable === true) {
        let is_rep_url = `${global4.ngrok_frontend_url}/checkout/repair/${product_id2}/`;
        is_rep_html = gen_func.render_user_state(user_state2, {
      ...
      $3(".product_repairable_cont").empty().append(is_rep_html);
      $3("#product_info_table_data_brand").text(product_details.brand_id);
      $3("#product_info_table_data_category").text(product_details.category_id);
...
`);
      ...
      $3("#product_models_content").empty().append(product_models_html_array);
      ...
      for (let product_config of product_configs) {
        ...
        let product_config_html = `<button type="button" class="btn global_remove_shadow global_red_select_btn_1 global_mobi_font_1 product_select_type" data-product-config-id="${product_config.product_config_id}" data-product-config-price="${product_config.price}">${product_config_text}</button>`;
        $3(product_config_html).appendTo(".product_selected_type_cont");
      }
    }).catch((error) => {
      console.log(error);
    });
  }
  var axios4, $3, add_cart_item2, check_user3, global4, render_func, product_func, user_state2;
  var init_products = __esm({
    "js/standard/products.js"() {
      init_shared_gen_func();
      axios4 = require_axios();
      $3 = require_jquery();
      ({ add_cart_item: add_cart_item2 } = (init_shopping_cart(), __toCommonJS(shopping_cart_exports)));
      ({ check_user: check_user3 } = (init_sens(), __toCommonJS(sens_exports)));
      ({ global: global4 } = require_config());
      $3(document).on("click", ".product_select_type", product_select_btn_type);
      $3(".product_price_qty_select").on("change", () => {
        let product_price = $3(".product_select_type.active").data("product-config-price");
        console.log(product_price);
        detect_amount_for_config_price(product_price);
      });
      $3(".product_add_to_cart").on("click", () => {
        let quantity = $3("#quantity_select option:selected").val();
        let product_config_id = $3(".product_select_type.active").data("product-config-id");
        const data2 = {
          quantity,
          product_config_id
        };
        add_cart_item2(data2);
      });
      render_func = {
        render_shipping: function(rate, city_name = null) {
          if (rate !== null) {
            $3(".product_shipping_city_name").text(city_name);
            $3(".product_shipping_rate_inner").text(rate);
          } else {
            $3(".shipping_rate_user_city").hide();
            $3(".product_shipping_rate").hide();
          }
        },
        render_products: function(data2) {
 ...
 <div>
          ...
          $3(".product_item_row").empty().append(product_html_array);
        }
      };
      product_func = {
        ...render_func,
        get_products: function(kwargs2 = null) {
          let url = `${global4.ngrok_api_url}/products/`;
          let query_param_list = [];
          query_param_list.length = 0;
          if (kwargs2 !== null) {
            for (const key of Object.keys(kwargs2)) {
              let prop = gen_func.get_prop(kwargs2, key);
              console.log(prop);
          ...
          console.log(url);
          axios4.get(url, global4.options).then((res) => {
            console.log(res);
      ...
      $3(".product_add_to_cart").on("click", () => {
        const product_config_id = $3(".product_select_type.active").data("product-config-id");
        const quantity = $3("#quantity_select option:selected").val();
        const data2 = {
          product_config_id,
          quantity
        };
      ...
      user_state2 = [false, false];
      $3(() => {
        user_state2 = check_user3();
  ...
  var filter_sort_methods_exports = {};
  __export(filter_sort_methods_exports, {
  ...
  function place_filter_options(type, type_id, type_value) {
    ...
    $4(ul_cont).empty();
    $4(html).appendTo(ul_cont);
    $4(`.${nav_line_class}`).last().remove();
    ...
    $4(ul_cont_mobi).empty();
    $4(html_mobi).appendTo(ul_cont_mobi);
    $4(`.${nav_line_class_mobi}`).last().remove();
  }
  function get_city_id() {
    let city_id = $4(".shop_options_location_option_btn.active").val();
    if (city_id === void 0) {
      city_id = null;
    }
    return city_id;
  ...
  function filter_on_change(e) {
    let sort_value;
    console.log($4(this));
    if ($4(this).prop("name") === "sort_option") {
      if ($4(this).is(":checked")) {
        $4(`input:checkbox[name='sort_option']`).not(this).prop("checked", false);
      }
    }
    kwargs = {};
    sort_value = $4(`input:checkbox[name='sort_option']:checked`).data("sort-value");
    if (sort_value !== null || void 0) {
      kwargs["sort"] = sort_value;
    }
    let brand_values = [];
    let category_values = [];
    $4(`input:checkbox[name='filter_option']:checked`).each(() => {
      let array;
      let filter_type = $4(this).data("filter-option-type");
      if (typeof filter_type !== "undefined") {
        ...
        array.push($4(this).data("filter-type-id"));
      }
    });
    ...
    product_func2.get_products(kwargs);
  }
  var axios5, $4, product_func2, global5;
  var init_filter_sort_methods = __esm({
    "js/standard/filter_sort_methods.js"() {
      axios5 = require_axios();
      $4 = require_jquery();
      ({ product_func: product_func2 } = (init_products(), __toCommonJS(products_exports)));
      ({ global: global5 } = require_config());
      $4(() => {
        axios5.get(`${global5.ngrok_api_url}/brands/`, {
          headers: global5.headers
        }).then((res) => {
          console.log(res);
        ...
        axios5.get(`${global5.ngrok_api_url}/categories/`, {
          headers: global5.headers
        }).then((res) => {
          console.log(res);
      ...
      $4(document).on("change", "input:checkbox", filter_on_change);
    }
  });
  ...
  var $5 = require_jquery();
  var { filter_on_change: filter_on_change2 } = (init_filter_sort_methods(), __toCommonJS(filter_sort_methods_exports));
  var { product_func: product_func3 } = (init_products(), __toCommonJS(products_exports));
  var { global: global6 } = require_config();
  $5(() => {
    axios6.get(`${global6.ngrok_api_url}/cities/`, global6.options).then((res) => {
      console.log(res);
      const cities = res.data;
      const cont2 = ".shop_options_location_select";
      ...
      for (let city of cities) {
        ...
        $5(`<button type="button" class="btn shop_options_location_option shop_options_location_option_btn global_font_3 global_remove_shadow" value="${city_id}">${city_value}</button>`).appendTo(cont2);
        $5(`<button type="button" class="btn shop_options_location_option_mobi shop_options_location_option_btn global_font_3 global_remove_shadow" value="${city_id}">${city_value}</button>`).appendTo(cont_mobi);
      }
    }).catch((err) => {
      console.log(err);
    });
  ...
  function change_on_selection(e) {
    $5(".shop_options_location_option_btn").not(this).removeClass("active");
    $5(this).addClass("active");
    let city_id = $5(this).val();
    const url = gen_func.url_id_check();
    const is_id = url[0];
    if (is_id) {
      const id = url[2];
      let axios_url = `${global6.ngrok_api_url}/products/${id}/get_shipping_rate_on_change?city_id=${city_id}`;
      axios6.get(axios_url, global6.options).then((res) => {
        data = res.data;
        product_func3.render_shipping(data.base_charge, data.city_value);
      }).catch((err) => {
        console.log(err);
      });
    } else {
  ...
  $5(document).on("click", ".shop_options_location_option_btn", change_on_selection);
})();
