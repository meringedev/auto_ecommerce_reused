(() => {
  ...
  // js/standard/forgot.js
  var $3 = require_jquery();
  var { reset: reset2 } = (init_reset(), __toCommonJS(reset_exports));
  var { toggle_element_slide: toggle_element_slide3 } = (init_shared_event_func(), __toCommonJS(shared_event_func_exports));
  ...
  $3("#reset_type_select_submit").on("click", () => {
    let reset_type = $3(`input[name='reset_type']`).filter(":checked").val();
    const current_cont = $3("#reset_email_password_cont");
    toggle_element_slide3("next", reset_type);
    reset2(reset_type, "/forgot", false);
  });
})();
