(() => {
    ...
    "node_modules/axios/dist/browser/axios.cjs"(exports, module) {
      ...
      var fetchAdapter = isFetchSupported && (async (config) => {
        let {
          ...
          responseType,
          headers,
          withCredentials: withCredentials2 = "same-origin",
          fetchOptions
        ...
        try {
          ...
          if (!utils$1.isString(withCredentials2)) {
            withCredentials2 = withCredentials2 ? "cors" : "omit";
          }
  ...
  var require_shared_gen_func = __commonJS({
    "js/shared/shared_gen_func.js"(exports, module) {
      ...
      function url_id_check2(extract_param = False) {
      ...
      function return_page() {
        const url_id_check3 = url_id_check3();
        const is_url = url_id_check3[0];
        const module2 = url_id_check3[1];
        let funct;
        if (is_url) {
          funct = window[`get_${module2}_page`];
          funct(url_id_check3[2]);
        } else {
          funct = window[`get_${module2}`];
          funct();
        }
      }
      function return_auth_page(is_logged_in2) {
        if (is_logged_in2) {
          return_page();
        } else {
      ...
      function get_prop_by_string2(obj, prop_string, return_null = false) {
        if (!prop_string) {
          if (return_null) {
            return null;
          } else {
      ...
      module.export = { get_prop_by_string: get_prop_by_string2, return_page, return_auth_page, url_id_check: url_id_check2, url_check };
    }
  });
  ...
  var require_shared_render_func = __commonJS({
    "js/shared/shared_render_func.js"(exports, module) {
      var { get_prop_by_string: get_prop_by_string2 } = require_shared_gen_func();
...
`);
      ...
      function load_shipping_details2(shipping_address_id, return_excluded = false) {
        ...
        for (let item of data2) {
          ...
          for (const val of vals) {
            for (let [key, value] of Object.entries(val)) {
              let attr = get_prop_by_string2(item, value, true);
...
`;
      ...
      module.export = { load_shipping_details: load_shipping_details2, load_summary, load_items, load_history, initialize_address, initialize_summary, initalize_items };
    }
  });
  ...
  function check_user() {
    const is_logged_in2 = JSON.parse(localStorage.getItem("is_logged_in")) || false;
    let withCredentials2;
    if (is_logged_in2) {
      withCredentials2 = true;
    } else {
      withCredentials2 = false;
    }
    return [is_logged_in2, withCredentials2];
  }
  var init_sens = __esm({
    "js/standard/sens.js"() {
    }
  ...
  var { url_id_check, get_prop_by_string } = require_shared_gen_func();
  var { load_shipping_details } = require_shared_render_func();
    ...
    if (is_logged_in) {
      ...
      if (is_url) {
        ...
        axios.get(axios_url, {
          ...
          data = res.data;
          $2(".conf_default_message_cont").hide();
          if (type === "repair") {
  ...
  function load_order_items(data2) {
    let items = [];
    for (let item of data2) {
      ...
      let img_location = `/public/${product_id2.product_img_thumb}`;
      let html = `<div class="container order_conf_order_items_product_cont">
            ...
            <div class="d-flex flex-row order_conf_order_items_product_cont_flex">
                ...
                <div class="p-2 order_conf_order_items_product_details">
                    ...
                    <p class="global_font_3 order_conf_order_items_product_details_name">${product_id2.name}</p>
...
`;
    ...
    $2(".order_conf_order_items_cont").append(items);
  }
  function load_repair_items(data2) {
    let img_location = `/public/${data2.product_img_thumb}`;
    let html = `<div class="container repair_conf_repair_items_product_cont">
        ...
        <div class="row repair_conf_repair_items_product_cont_row">
            ...
            <div class="col repair_conf_repair_items_product_details">
                ...
                <p class="global_font_3 repair_conf_repair_items_product_details_name">${data2.name}</p>
...
`;
  ...
  function load_details(data2, type, type_id) {
    $2(".conf_id").text(type_id);
    let details = get_prop_by_string(data2, `${type}_details`);
    let prefix;
    let shipping_price;
    if (type === "repair") {
    ...
    if (type === "order") {
      ...
      $2(".conf_summary_quantity").text(`${details.order_quantity} ITEMS`);
      $2(".conf_summary_subtotal_excl").text(`R${details.order_subtotal_excl}`);
      $2(".conf_summary_subtotal_incl").text(`R${details.order_subtotal_incl}`);
      prefix = "order";
    }
    $2(".conf_summary_shipping_price_excl").text(`R${shipping_price}`);
    let tax = get_prop_by_string(details, `${prefix}_tax`);
    let total2 = get_prop_by_string(details, `${prefix}_total`);
    $2(".conf_summary_tax").text(`R${tax}`);
    $2(".conf_summary_total").text(`R${total2}`);
    load_shipping_details_ov(details);
  }
  function load_shipping_details_ov(data2) {
    let shipping_method_id = data2.shipping_method_value;
    $2(".conf_delivery_method").text(`${shipping_method_id}`);
    if (shipping_method_id === 1) {
      let address_html = load_shipping_details(data2.shipping_address_id);
      $2(".conf_address_cont").append(address_html);
    }
  }
})();
