(() => {
  ...
  var require_shared_render_func = __commonJS({
    "js/shared/shared_render_func.js"(exports, module) {
      ...
      function initialize_address(shipping_address_id2) {
        const excluded = ["address_id", "user_id", "name", "is_active", "is_default"];
        let address2 = [];
        let excluded_values2 = [];
        for (let [key, value] of Object.entries(shipping_address_id2)) {
          if (value !== null) {
...
`);
      ...
      function load_shipping_details(shipping_address_id2, return_excluded = false) {
        [address, excluded_values] = initialize_address(shipping_address_id2);
...
`;
      ...
      function initialize_summary(data, type) {
        let extra2 = null;
        let shipping;
        let subtotal;
        let tax;
      ...
      function load_summary(data, type) {
        let template = '<tbody class="item_summary_body"></tbody>'[fields, extra, total] = initialize_summary(data, type);
        let rows = [];
        if (extra !== null) {
          rows.push(
    ...
    <td class="my_account_main_indi_item_price_summary_table_data global_font_italic_bold my_account_main_indi_item_price_summary_table_total">R${total}</td>`
      ...
      function initalize_items(data, repair = false) {
        let items = [];
        let product_id2;
        let vals = [
      ...
      function load_items(data, repair = false) {
        const items = initalize_items(data, repair);
        let item_html = [];
...
`;
  ...
  // js/standard/my_account_address_book.js
  var require_my_account_address_book = __commonJS({
    "js/standard/my_account_address_book.js"(exports) {
      var $2 = require_jquery();
      var axios = require_axios();
      ...
      $2(() => {
        ...
        if (is_logged_in) {
          $2(".my_account_main_item_cont").remove();
          const url = `http://host.docker.internal:3000/user/addresses`;
          axios.get(url, {
            withCredentials
          }).then((res) => {
            const user_addresses = res.data;
            const excluded = ["address_id", "user_id", "name", "is_active", "is_default"];
            for (let address2 of user_addresses) {
              let address_details = [];
              let excluded_values2 = [];
              ...
              const is_default = excluded_values2[4];
              let is_default_html;
...
`;
                ...
                <div class="container my_account_main_item_cont" data-address-id="${address_details[0]}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-12 col-md-8 col-lg-7 col-xl-7 global_font_4 my_account_main_item_address_details_cont">
                            ...
                            <p class="global_font_4 global_font_italic" data-key="name">${excluded_values2[2]}</p>
      ...
      $2(".my_account_address_book_set_default_btn").on("click", () => {
        const address_id = $2(exports).closest("[data-address-id]");
        if (address_id !== void 0) {
          axios.patch(`http://host.docker.internal:3000/user/addresses/${address_id}/make_default`, { withCredentials }).then((res) => {
            location.reload();
          }).catch((err) => {
            console.log(err);
          });
      ...
      $2(".my_account_address_book_edit_btn").on("click", () => {
        const address_id = $2(exports).closest("[data-address-id]");
        let address_details = {};
        let address_name = null;
        address_id.children(".my_account_main_item_address_details_cont").children("p").each((index) => {
          if (index === 0) {
            let data_attr = $2(exports).data("key");
            if (data_attr === "name") {
              address_name = data_attr.text();
            } else {
              $2(exports).children("span").each(() => {
                let data_attr2 = $2(exports).data("key");
                address_details.append(data_attr2, data_attr2.text());
              });
            }
          }
        });
        const address_popup = $2(".my_account_add_address_popup1");
        ...
        $2(".my_account_add_address_input").on("click", () => {
          $2(exports).data("clicked", true);
        });
        $2("#my_account_add_address_submit").on("click", () => {
          let data = {};
          $2("my_account_add_address_input").each(() => {
            let is_clicked = $2(exports).data("clicked");
            if (is_clicked) {
              let key = $2(exports).data("field-name");
              data.append(key, key.val());
            }
          });
          axios.patch(`http://host.docker.internal:3000/user/addresses/${address_id}/`, {
            withCredentials,
            data
          }).then((res) => {
      ...
      $2(".my_account_address_book_add_address_btn").on("click", () => {
        const address_popup = $2(".my_account_add_address_popup1");
        address_popup.modal("show");
        $2("#my_account_add_address_submit").on("click", () => {
          let data = {};
          ...
          axios.post("http://host.docker.internal:3000/user/addresses/", {
            withCredentials: true,
            data
          }).then((res) => {
            console.log(res.data.message);
  ...
  require_my_account_address_book();
})();
