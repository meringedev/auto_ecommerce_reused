(() => {
  ...
  function reset(reset_type, url, withCredentials) {
    ...
    if (reset_type === "password") {
      $2("#reset_select_method_submit").on("click", () => {
        method = $2(`input[name='reset_select_method']`).filter(":checked").val();
        $2("#reset_select_method_cont").slideUp();
      });
    }
    if (method !== null || void 0) {
      user_field = method;
      if (method === "SMS") {
        user_field = "mobile_no";
      }
    }
    const verify_cont = $2(`#verify_${user_field}_method_cont`);
    verify_cont.slideDown();
    $2(`#verify_${user_field}_method_submit`).on("click", () => {
      let user_field_data = $2(`input[name='verify_${user_field}_method_input']`).val();
      if (user_field_data === null || void 0) {
        const data2 = {
          method,
          reset_type,
        ...
        axios.post(`${url}/reset/`, {
          ...
          verify_cont.slideUp();
          const code_cont = $2("#reset_input_code_cont");
          code_cont.slideDown();
          $2("#reset_input_code_submit").on("click", () => {
            let user_input_otp = $2(`input[name='reset_input_code']`).val();
            ...
            axios.post(`${url}/verify_otp/`, {
              ...
              code_cont.slideUp();
              $2(`reset_input_new_${reset_type}_cont`).slideDown();
              $2(`reset_input_new_${reset_type}_submit`).on("click", () => {
                const input_new = $2(`input[name='reset_new_${reset_type}_input']`).val();
                const input_conf = $2(`input[name='reset_conf_${reset_type}_input']`).val();
                const input_data = {
  ...
  var $2, axios, toggle_element_slide2;
  var init_reset = __esm({
    "js/standard/reset.js"() {
      $2 = require_jquery();
      axios = require_axios();
      ({ toggle_element_slide: toggle_element_slide2 } = (init_shared_event_func(), __toCommonJS(shared_event_func_exports)));
    }
  });
  ...
  // js/standard/my_account_details.js
  var require_my_account_details = __commonJS({
    "js/standard/my_account_details.js"(exports) {
      var $3 = require_jquery();
      var axios2 = require_axios();
      var { reset: reset2 } = (init_reset(), __toCommonJS(reset_exports));
      ...
      $3(() => {
        ...
        if (is_logged_in) {
          axios2.get("/user/details/", { withCredentials }).then((res) => {
            data = res.data;
            for (let [key, value] of Object.entries(data)) {
              let cont = $3(`[data-field-type='${key}']`);
      ...
      $3(".my_account_details_edit_btn").on("click", () => {
        const field_type = $3(exports).closest("[data-field-type]");
        const popup = `my_account_details_${field_type}_popup`;
        $3(popup).modal("show");
        if (field_type === "email" || "password") {
          $3(".my_account_details_reset").on("click", () => {
            $3(popup).modal("hide");
            $3(".my_account_details_reset_popup").modal("show");
            if (field_type === "password") {
              $3("#reset_select_method_cont").show();
            }
            if (field_type === "email") {
              $3("#verify_mobile_no_method_cont").show();
            }
            reset2(field_type, "/user/details", withCredentials);
          });
        } else {
          const initial_value = $3(exports).closest(".my_account_initial_value").text();
          $3(".my_account_details_submit").on("click", () => {
            let changed_value = $3(`input[name='my_account_details_edit_${field_type}']`).val();
            if (changed_value !== initial_value) {
              const data2 = { [field_type]: changed_value };
              axios2.patch("/user/details/", {
                data: data2,
                withCredentials
              }).then((res) => {
