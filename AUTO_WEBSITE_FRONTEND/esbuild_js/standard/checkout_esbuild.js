(() => {
  ...
  // js/standard/checkout.js
  var require_checkout = __commonJS({
    "js/standard/checkout.js"(exports) {
      var axios = require_axios();
      var $ = require_jquery();
      var { get_prop_by_string, url_id_check } = require_shared_gen_func();
      ...
      $(() => {
        ...
        if (is_logged_in) {
          ...
          axios.get(axios_url, { withCredentials }).then((res) => {
            const data2 = res.data;
            const checkout_total = data2.checkout_total;
            const checkout_items = data2.checkout_items;
            const user_addresses = data2.user_addresses;
            const active_address = data2.active_address.address_id;
            if (module_type === "repair") {
              load_checkout_items(checkout_items, repair = true);
              load_exchange_units(data2.checkout_contains_ex);
              const repair_saved = ["reason_repair", "error_codes"];
              for (let repair_val of repair_saved) {
                const data_val = get_prop_by_string(data2, `saved_${repair_val}`, true);
                if (data_val !== null) {
      ...
      function load_checkout_items(data2, repair2 = false) {
...
`;
        ...
        `;
          checkout_items_html_array.push(html2);
        }
        $(".checkout_items_cont").append(checkout_items_html_array);
      }
      function load_user_addresses(data2, active_address_id) {
          ...
          for (let [key, value] of Object.entries(item)) {
            if (value !== null) {
              if (excluded.some((i) => key.includes(i))) {
                ...
                if (active_address_id === data2.address_id) {
                  is_active = true;
                }
...
`);
          ...
          let inner_address_html = `<p>${name}</p>
        ...
        <p>${address}</p>`;
          ...
          let address_html = `<div class="${address_html_class}" data-shipping-address-id="${address_id}">
      ...
      function load_checkout_total(data2) {
        let summary = [];
        ...
        for (let [key, value] of Object.entries(data2)) {
          if (value !== null || 0) {
            if (excluded.some((i) => key.includes(i))) {
              ...
              let html_row = `<tr class="checkout_summary_table_row">
...
`;
        ...
        if (excluded_values.length === 3) {
          extra_html = `<tr class="checkout_summary_table_row">
...
`;
    ...
    <tr class="checkout_summary_table_row">
        ...
        $("table.checkout_summary_table > tbody").append(html2);
      }
      function load_exchange_units(data2) {
        if (data2 === true) {
      ...
      $("#checkout_exchange_unit_upload").on("change", () => {
        ...
        let file_data = $(exports).prop("files")[0];
        let params2 = [
          { "user_type": "user" },
          { "instance_type": "order" }
        ];
        ...
        axios.post("http://host.docker.internal:3000/upload", {
          ...
          if (status === 200) {
            ...
            let html2 = `<p class="checkout_exchange_unit_photo_file_name global_font_italic" data-internal-filename="${filename}">${user_filename}<button type="button" class="btn global_remove_shadow global_black_icon_btn checkout_exchange_unit_photo_file_remove"><i class="fa-solid fa-trash fa-sm"></i></button></p>`;
            $(".checkout_exchange_unit_photo_file_cont").append(html2);
          }
        });
      });
      $(".checkout_exchange_unit_photo_file_remove").on("click", () => {
        ...
        $(`${btn}_submit`).on("click", () => {
          ...
          axios.delete(axios_url, { withCredentials }).then((res) => {
            if (res.status === 200) {
              file.hide();
            }
          }).catch((err) => {
      ...
      $(".checkout_delivery_method_btns").on("click", () => {
        ...
        $(".checkout_delivery_method_btns").not(exports).removeClass("active");
        $(exports).addClass("active");
        let shipping_method_id = $(exports).data("shipping_method_id");
      ...
      $("#checkout_summary_payment_btn").on("click", () => {
        ...
        if (module === "order") {
          ...
          data2.append("ex_unit_images", filenames);
        }
        if (module === "repair") {
